 ...
 * \file NSSolverFNFDM.h
 * \brief NSSolverFNFDM class, used to solve the NS equation in a flexible node grid.
 * \author Chen Jie.
 *
...
{
  public:
    NSSolverFNFDM(index_type index, string name, shared_ptr<FlowSolverParam> para, shared_ptr<GridFN> grid,
  ...
  protected:
    ...
    void BCInflow(dynamic_array<BoundFN> &bound_list);
    // 超声速出口边界条件
    virtual void BCOutflow(BoundFN &bound);
    void BCOutflow(dynamic_array<BoundFN> &bound_list);
    // 壁面边界条件
