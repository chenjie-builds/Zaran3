...
namespace zaran
{
    NodeMetric::NodeMetric(GridFN* grid)
    {
        int nNode = grid->GetTotalNodeNum();
        m_metric = new double[nNode * (4 * 4 + 1)];
        m_grid = grid;
    ...
    const double NodeMetric::GetJacobian(int iNode) const
    {
        return m_metric[iNode * 17 + 16];
    }

    const double NodeMetric::GetMaxJacobian() const
    {
        return max_jacobian;
    }

    const double NodeMetric::GetMinJacobian() const
    {
        return min_jacobian;
    }

    const int NodeMetric::GetMaxJacobianNode() const
    {
        return max_jacobian_node;
    }

    const int NodeMetric::GetMinJacobianNode() const
    {
        return min_jacobian_node;
    }

    void NodeMetric::CalcMetric(double* metric, const double* xRight, const double* xLeft, const double* yRight, const double* yLeft, const double* zRight, const double* zLeft)
    {
        double coef_x[4], coef_y[4], coef_z[4], coef_t[4];
        double* coef_xi = metric;
        double* coef_eta = metric + 4;
        double* coef_zeta = metric + 8;
        double* coef_tau = metric + 12;
        double& jacobian = metric[16];
    ...
    bool NodeMetric::ErrorJacobi(double jacobi) const
    {
        if (jacobi <= 0 || isnan(jacobi) || isinf(jacobi))
        {
            return true;
        }
        return false;
