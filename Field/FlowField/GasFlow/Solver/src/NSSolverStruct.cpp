    ...
    for (int k = 0; k < nk; ++k)
    {
        for (int j = 0; j < nj; ++j)
        {
            for (int i = 0; i < ni; ++i)
            {
                ...
                // if (x <= 1.0 / 6.0 || y >= (x - 1.0 / 6.0) * tan(PI / 3.0))
                if (x <= 0.10001)
                {
                    data_manager->SetPrim(0, idx, 6.4);
                    data_manager->SetPrim(1, idx, 3.125);
                    data_manager->SetPrim(2, idx, 0);
                    data_manager->SetPrim(3, idx, 0.0);
                    data_manager->SetPrim(4, idx, 18.5);
                }
                else
                {
                    data_manager->SetPrim(0, idx, 1.4);
...
void NSSolverStruct::CalcJacobian()
{
    ...
    {
        Log::warn("Jacobian Scheme is not defined, use V1 as default");
        CalcJacobianV1();
    }
}
                ...
                for (int i = is; i <= ie; ++i)
                {
                    ...
                    for (int idx_eq = 0; idx_eq < 5; ++idx_eq)
                    {
                        data_manager->SetCons(idx_eq, idx,
                                              data_manager->GetConsOld(idx_eq, idx) +
                                                  dt * rk_coef[iStage] * data_manager->GetResidual(idx_eq, idx) *
                                                      jacobi);
                    }
                }
            }
        ...
        for (int j = js - 1; j <= je + 1; ++j)
        {
            for (int i = is - 1; i <= ie + 1; ++i)
            {
                ...
                for (int idx_eq = 0; idx_eq < equ_num; ++idx_eq)
                {
                    ...
                    InterPrimMidNodeWCNS5(value_temp, left_value, right_value);
...
// 参考《计算空气动力学》pp.84
void NSSolverStruct::InterPrimMidNodeWCNS5(const double *value, double &value_left, double &value_right)
{
    double h = 0.025;
...
void NSSolverStruct::WallBC(BoundStruct &bound)
{
    ...
    // Log::info("wall: ({},{},{}), norm = ({},{},{})", i_bound, j_bound, k_bound, norm_bnd[0], norm_bnd[1],
    // norm_bnd[2]);
    for (int iGhost = 1; iGhost <= ghost_size; ++iGhost)
    {
