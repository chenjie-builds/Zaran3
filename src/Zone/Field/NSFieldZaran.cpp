...
NSFieldZaran::~NSFieldZaran() = default;
...
void NSFieldZaran::SetModelManager(const shared_ptr<ModelManager> &model_manager)
{
    m_model_manager = model_manager;
}

shared_ptr<zaran::GridBlock> NSFieldZaran::GetGrid() const
{
...
{
    count_type ni = GetGrid()->GetNi();
    count_type nj = GetGrid()->GetNj();
    count_type nk = GetGrid()->GetNk();
...
void NSFieldZaran::CalcResidual()
{
    ...
    count_type total_node_num = (ie - is + 1) * (je - js + 1) * (ke - ks + 1);
    ...
    for (int iEqu = 0; iEqu < equ_num; iEqu++)
    {
        ...
        norm_l2 = 0;
        const auto res = GetDataManager()->GetResidual(iEqu);
        for (int k = ks; k <= ke; k++)
        {
            for (int j = js; j <= je; j++)
            {
...
void NSFieldZaran::DeleteSlaveField(const shared_ptr<FieldManager> &field_manager) const
{
    ...
    const auto data_comm = field_manager->GetFieldDataCommInfo(this->GetIdx());
    const auto src_node_num = data_comm->GetRecvNodeNum();
...
void NSFieldZaran::CreateSlaveField(const shared_ptr<FieldManager> &field_manager)
{
    ...
    dimension_type dim = grid_block->GetDim();
    ...
    const auto &fn_grid_info = grid_factory.GetFNGridInfo();
    const auto &ref_node = grid_factory.GetRefNode();
    index_type fn_recv_num = ref_node.size();
    ...
    index_type block_recv_num = trans_node.size();
...
//         auto node = grid->GetNode();
//         m_idx_proxy = new IdProxyStruct(grid);
