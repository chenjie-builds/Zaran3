...
namespace zaran
{
NSSolverStruct::NSSolverStruct(index_type index, string name, shared_ptr<FlowSolverParamStruct> para,
                               shared_ptr<GridStruct> grid, shared_ptr<DataManagerNSStruct> data_manager)
    ...
    for (int k = 0; k < nk; ++k)
    {
        for (int j = 0; j < nj; ++j)
        {
            for (int i = 0; i < ni; ++i)
            {
                ...
                prim[1] = beta * exp(0.5 * (1.0 - r2)) / (2.0 * PI) * (-y + yc);
                prim[2] = beta * exp(0.5 * (1.0 - r2)) / (2.0 * PI) * (x - xc);
                prim[3] = 0.0;
    ...
    for (index_type k = 0; k < nk; ++k)
    {
        for (index_type j = 0; j < nj; ++j)
        {
            for (index_type i = 0; i < ni; ++i)
            {
                auto idx = idx_proxy(i, j, k);
                auto coord = node->GetCoord(idx);
                double dist = sqrt(pow(coord[0] - explosion_center[0], 2) + pow(coord[1] - explosion_center[1], 2) +
    ...
    for (int k = 2; k < nk - 2; ++k)
    {
        for (int j = 2; j < nj - 2; ++j)
        {
            for (int i = 2; i < ni - 2; ++i)
            {
                ...
                temp_i[2] = i + 1, temp_j[2] = j, temp_k[2] = k;
                temp_i[3] = i + 2, temp_j[3] = j, temp_k[3] = k;
                temp_i[4] = i, temp_j[4] = j - 2, temp_k[4] = k;
                temp_i[5] = i, temp_j[5] = j - 1, temp_k[5] = k;
                ...
                for (int idx_eq = 0; idx_eq < 5; ++idx_eq)
                {
                    ...
                    {
                        double delta_val =
                            data_manager->GetPrim(idx_eq, idx_proxy(temp_i[iNeigh], temp_j[iNeigh], temp_k[iNeigh])) -
...
#pragma omp parallel for collapse(3) private(A, A_inv, b, grad, temp_i, temp_j, temp_k)
    ...
    for (int k = 1; k < nk - 1; ++k)
    {
        for (int j = 1; j < nj - 1; ++j)
        {
            for (int i = 1; i < ni - 1; ++i)
            {
                // Log::info("Calculating primitive gradient at ({}, {}, {})", i, j, k);
                A.setZero();
                int idx = idx_proxy(i, j, k);
                auto coord = node->GetCoord(i, j, k);
                temp_i[0] = i - 1, temp_j[0] = j, temp_k[0] = k;
                ...
                for (int iNeigh = 0; iNeigh < neighbor_num; ++iNeigh)
                {
                    ...
                    double delta_x = coord_neigh[0] - coord[0];
                    double delta_y = coord_neigh[1] - coord[1];
                    double delta_z = coord_neigh[2] - coord[2];
                    double weight = 1.0 / sqrt(delta_x * delta_x + delta_y * delta_y + delta_z * delta_z);
                    A(0, 0) += weight * delta_x * delta_x;
                    A(0, 1) += weight * delta_x * delta_y;
    ...
    // Log::info("Primitive gradient calculation completed.");
}

void NSSolverStruct::CalcLimiter()
{
    ...
    {
        Log::error("Limiter type is not supported!");
    }
}
            ...
            for (int j = 1; j < nj - 1; ++j)
            {
                for (int i = 1; i < ni - 1; ++i)
                {
                    ...
                    for (int iNeigh = 0; iNeigh < neighbor_num; ++iNeigh)
                    {
                        ...
                        min_dis = Min(min_dis, DistanceOfTwoPoints(node->GetCoord(idx), node->GetCoord(idx_neigh)));
                    }
                    // eps = vk_coef * (max_val - min_val) + SMALL_NUMBER;
                    // eps = eps * eps;
                    double eps = pow(vk_coef * min_dis, 3);
...
void NSSolverStruct::CalcLimiterFirstOrder()
{
    auto grid = GetGrid();
    auto data_manager = GetDataManager();
    IdProxyStruct &idx_proxy = GetIdxProxy();
...
void NSSolverStruct::InterMidNodePrim_Grad()
{
    ...
    double value_left[5], value_right[5];
#ifdef USE_OMP
#pragma omp parallel for collapse(3) private(value, coord_vec, value_left, value_right)
#endif // USE_OMP
    for (int k = 1; k < nk - 1; ++k)
    {
        for (int j = 1; j < nj - 1; ++j)
                ...
                for (dimension_type iDim = 0; iDim < grid->GetDim(); ++iDim)
                {
                    ...
                    for (int idx_eq = 0; idx_eq < equ_num; ++idx_eq)
                    {
                        for (int iTemp = 0; iTemp < 5; ++iTemp)
                        {
                            ...
                            for (int jDim = 0; jDim < 3; ++jDim)
                            {
                                coord_temp[iTemp][jDim] =
                                    node->GetCoord(i_temp[iTemp], j_temp[iTemp], k_temp[iTemp])[jDim];
                            }
                        }
        ...
        for (int j = js - 1; j <= je + 1; ++j)
        {
            for (int i = is - 1; i <= ie + 1; ++i)
            {
                for (int iDim = 0; iDim < grid->GetDim(); ++iDim)
                {
                    ...
                    {
                        idx_temp[iTemp] =
                            idx_proxy(i + direction[iDim][0] * (iTemp - 2), j + direction[iDim][1] * (iTemp - 2),
                                      k + direction[iDim][2] * (iTemp - 2));
                    }
                    double value_base[5];
                    ...
                    // if (left_value[0] < min_density || left_value[4] < min_pressure || left_value[0]>max_density ||
                    // left_value[4]>max_pressure)
                    //{
                    //	for (int idx_eq = 0; idx_eq < equ_num; ++idx_eq)
                    //	{
                    ...
                    //	}
                    // }
                    // if (right_value[0] < min_density || right_value[4] < min_pressure || right_value[0]>max_density
                    // || right_value[4]>max_pressure)
                    //{
                    //	for (int idx_eq = 0; idx_eq < equ_num; ++idx_eq)
                    //	{
                    ...
                    for (int idx_eq = 0; idx_eq < equ_num; ++idx_eq)
                    {
                        data_manager->SetMidNodePrimLeft(idx_eq, iDim, idx_temp[2],
                                                         left_value[idx_eq] * value_base[idx_eq]);
                        data_manager->SetMidNodePrimRight(idx_eq, iDim, idx_temp[1],
                                                          right_value[idx_eq] * value_base[idx_eq]);
                    }
                }
            }
        }
...
void NSSolverStruct::InterGhostMidNodePrim_WCNS5()
{
    ...
    index_type i, j, k;
#ifdef USE_OMP
#pragma omp parallel for collapse(2) private(j, k, idx_temp_left, idx_temp_right, value_left, value_right, left_value, \
                                                 right_value)
#endif // USE_OMP
    for (k = ks; k <= ke; ++k)
    {
...
#ifdef USE_OMP
#pragma omp parallel for collapse(2) private(i, k, idx_temp_left, idx_temp_right, value_left, value_right, left_value, \
                                                 right_value)
#endif // USE_OMP
    for (k = ks; k <= ke; ++k)
    {
    ...
    {
#ifdef USE_OMP
#pragma omp parallel for collapse(2) private(i, j, idx_temp_left, idx_temp_right, value_left, value_right, left_value, \
                                                 right_value)
#endif // USE_OMP
        for (j = js; j <= je; ++j)
        {
                                           ...
                                           double *value_right)
{
    ...
    for (int idx_eq = 0; idx_eq < equ_num; ++idx_eq)
    {
        value_left[idx_eq] = data_manager->GetPrim(idx_eq, idx_left) +
                             0.5 * data_manager->GetLimiter(idx_eq, idx_left) *
        ...
        value_right[idx_eq] = data_manager->GetPrim(idx_eq, idx_right) -
                              0.5 * data_manager->GetLimiter(idx_eq, idx_right) *
...
void NSSolverStruct::BoundaryCondition()
{
    ...
    for (auto &boundary : bound_map->GetBoundMap())
    {
        ...
        if (bound_name == "hole")
            continue;
        if (bound_name == "riemann" || bound_name == "farfield")
        {
            BCFarfield(bound);
        }
...
#pragma omp parallel for private(prim_vals, cons_vals)
    ...
    for (size_t iBound = 0; iBound < bound.size(); ++iBound)
    {
        ...
        GetGas()->Prim2Cons(prim_vals, cons_vals);
        for (size_t iGhost = 1; iGhost <= ghost_size; ++iGhost)
        {
            i_ghost = i_bound + iGhost * bound_direction[0];
            j_ghost = j_bound + iGhost * bound_direction[1];
...
#pragma omp parallel for
    ...
    for (size_t iBound = 0; iBound < bound.size(); ++iBound)
    {
        ...
        for (size_t iter_ghost_level = 1; iter_ghost_level <= ghost_level; ++iter_ghost_level)
        {
            ...
            double vel_ref[3] = {data_manager->GetPrim(1, idx_ref), data_manager->GetPrim(2, idx_ref),
...
void NSSolverStruct::BCVortex(dynamic_array<BoundStruct> &bound)
{
    ...
    for (size_t iBound = 0; iBound < bound.size(); ++iBound)
    {
        ...
        for (size_t iGhost = 1; iGhost <= ghost_size; ++iGhost)
        {
            ...
            prim[0] =
                pow(1.0 - (gamma - 1.0) * beta * beta * exp(1.0 - r2) / (8.0 * gamma * PI * PI), 1.0 / (gamma - 1.0));
            prim[4] = pow(prim[0], gamma);
            prim[1] = beta * exp(0.5 * (1.0 - r2)) / (2.0 * PI) * (-coord[1] + yc);
            prim[2] = beta * exp(0.5 * (1.0 - r2)) / (2.0 * PI) * (coord[0] - xc);
            prim[3] = 0.0;
