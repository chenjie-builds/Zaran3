...
namespace zaran
{
NSSolverStructDEER::NSSolverStructDEER(index_type index, string name, shared_ptr<FlowSolverParamStruct> para,
...
// #ifdef USE_OMP
// #pragma omp parallel for private(idx, idx_temp, value, res_tmp, mid_coord_left, mid_coord_right,
// mid_coord,move_vector, riemann_para) #endif // USE_OMP
//
            ...
            for (int i = is; i <= ie; ++i)
            {
                ...
                for (dimension_type dim = 0; dim < grid->GetDim(); dim++)
                {
                    for (int iTemp = 0; iTemp < 4; iTemp++)
                    {
                        ...
                        riemann_para[iTemp].nt = node_metrics->GetMetrics(dim, idx)[3];
                        int idx_temp =
                            idx_proxy(i + (iTemp - 2) * direction[dim][0], j + (iTemp - 2) * direction[dim][1],
                    ...
                    for (int idx_eq = 0; idx_eq < equ_num; ++idx_eq)
                    {
                        res_tmp[idx_eq] -= coef1 * (riemann_para[2].flux[idx_eq] - riemann_para[1].flux[idx_eq]) +
        ...
        for (int j = js; j <= je; ++j)
        {
            for (int i = is; i <= ie; ++i)
            {
                ...
                for (int dim = 0; dim < grid->GetDim(); ++dim)
                {
                    ...
                    for (int idx_eq = 0; idx_eq < equ_num; ++idx_eq)
                    {
                        res_tmp[idx_eq] -= coef1 * (riemann_para[3].flux[idx_eq] - riemann_para[2].flux[idx_eq]) +
...
// void NSSolverStructDEER::InterMidNodePrim_Grad(int idx_left, int idx_right, const double* lef_coord, const double*
// mid_coord, 	const double* right_coord, double* value_left, double* value_right)
//{
