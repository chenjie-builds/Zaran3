...
void Visual::WriteTecplot(Ptr<FieldSolver>& solver)
{
	...
	fout << "variables=x,y,z,rho,u,v,w,p,limitCoef0,limitCoef1,limitCoef2,limitCoef3,limitCoef4,rhoGradX,rhoGradY,rhoGradZ,uGradX,uGradY,uGradZ,vGradX,vGradY,vGradZ,wGradX,wGradY,wGradZ,pGradX,pGradY,pGradZ\n";
	...
	auto& limitCoef0 = data->GetData("limiterCoef0");
	auto& limitCoef1 = data->GetData("limiterCoef1");
	auto& limitCoef2 = data->GetData("limiterCoef2");
	auto& limitCoef3 = data->GetData("limiterCoef3");
	auto& limitCoef4 = data->GetData("limiterCoef4");
	auto& rhoGradX = data->GetData("rhoGradX");
	auto& rhoGradY = data->GetData("rhoGradY");
	auto& rhoGradZ = data->GetData("rhoGradZ");
	auto& uGradX = data->GetData("uGradX");
	auto& uGradY = data->GetData("uGradY");
	auto& uGradZ = data->GetData("uGradZ");
	auto& vGradX = data->GetData("vGradX");
	auto& vGradY = data->GetData("vGradY");
	auto& vGradZ = data->GetData("vGradZ");
	auto& wGradX = data->GetData("wGradX");
	auto& wGradY = data->GetData("wGradY");
	auto& wGradZ = data->GetData("wGradZ");
	auto& pGradX = data->GetData("pGradX");
	auto& pGradY = data->GetData("pGradY");
	auto& pGradZ = data->GetData("pGradZ");
	...
	for (int iNode = 0; iNode < grid->GetTotalNodeNum(); ++iNode)
	{
		...
		fout << limitCoef0[iNode] << "  " << limitCoef1[iNode] << "  " << limitCoef2[iNode] << "  " << limitCoef3[iNode] << "  " << limitCoef4[iNode];
		fout << "  " << rhoGradX[iNode] << "  " << rhoGradY[iNode] << "  " << rhoGradZ[iNode];
		fout << "  " << uGradX[iNode] << "  " << uGradY[iNode] << "  " << uGradZ[iNode];
		fout << "  " << vGradX[iNode] << "  " << vGradY[iNode] << "  " << vGradZ[iNode];
		fout << "  " << wGradX[iNode] << "  " << wGradY[iNode] << "  " << wGradZ[iNode];
		fout << "  " << pGradX[iNode] << "  " << pGradY[iNode] << "  " << pGradZ[iNode];
		fout << std::endl;
	}
