...
namespace zaran {
	...
	void GridFNFactoryZaran::ProcessCell(int start_i, int end_i, int start_j,
		...
		double tol1 = tol_factor * sqrt(grid->GetDx() * grid->GetDx() +
			grid->GetDy() * grid->GetDy() +
			grid->GetDz() * grid->GetDz());
		tol += tol1;
		PhysicalType cell_type = PhysicalType::Unset;
		double dist = 0;
		if (x_max < model_box.x_min - tol1 || x_min > model_box.x_max + tol1 ||
			y_max < model_box.y_min - tol1 || y_min > model_box.y_max + tol1 ||
			z_max < model_box.z_min - tol1 || z_min > model_box.z_max + tol1) {
	...
	void GridFNFactoryZaran::WriteProjectNode() {
		...
		for (auto& node : m_fn_info.node[m_layer_num]) {
			out << node.coord[0] << " " << node.coord[1] << " " << node.coord[2]
				<< std::endl;
		}
	}
	...
	void GridFNFactoryZaran::WriteModelSurface() {
		...
		out << "variables = x, y, z" << std::endl;
		out << "Zone T=test" << std::endl;
		out << " N=" << m_fn_info.node[m_layer_num].size()
			<< " E=" << m_trans_face.size() << " ZONETYPE=FEQuadrilateral"
			<< std::endl;
		out << "DATAPACKING=POINT" << std::endl;
		for (auto& coord : m_fn_info.node[m_layer_num]) {
			out << coord.coord[0] << " " << coord.coord[1] << " " << coord.coord[2]
				<< std::endl;
		}
	...
	void GridFNFactoryZaran::WriteTransFace() {
		...
		out << "variables = x, y, z" << std::endl;
		out << "Zone T=test" << std::endl;
		out << " N=" << m_fn_info.node[1].size() << " E=" << m_trans_face.size()
	...
	void GridFNFactoryZaran::WriteSlaveGrid() {
		...
		out << " N=" << node_num << " E=" << cell_num << " F=FEPOINT, ET=Brick"
			<< std::endl;
		for (int iLayer = 0; iLayer < m_fn_info.node.size(); iLayer++) {
			for (int iNode = 0; iNode < m_fn_info.node[iLayer].size(); iNode++) {
				out << m_fn_info.node[iLayer][iNode].coord[0] << " "
					<< m_fn_info.node[iLayer][iNode].coord[1] << " "
					<< m_fn_info.node[iLayer][iNode].coord[2] << std::endl;
			}
		}
	...
	bool GridFNFactoryZaran::CheckTransNode() {
		...
		if (total_error_num == 0) {
			for (int iNode = 0; iNode < m_node_type.size(); iNode++) {
				...
				double tol_factor = GlobalData::GetDouble("tol_factor");
				double tol = tol_factor * sqrt(grid->GetDx() * grid->GetDx() +
					grid->GetDy() * grid->GetDy() +
					grid->GetDz() * grid->GetDz());
				if (dist < tol) {
					m_cell_type[m_idx_proxy->GetIdx(i, j, k)] = PhysicalType::Solid;
	...
	void GridFNFactoryZaran::BuildWallNode() {
		...
		for (int iNode = 0; iNode < m_fn_info.node[1].size(); iNode++) {
			...
			m_fn_info.node[m_layer_num][iNode].idx =
				iNode + m_fn_info.node[0].size() +
				m_fn_info.node[1].size() * (m_layer_num - 1);
		}
	}
	...
	void GridFNFactoryZaran::BuildProjectNode() {
		...
		for (int iNode = 0; iNode < m_fn_info.node[1].size(); iNode++) {
			...
			for (int iLayer = 2; iLayer < m_layer_num; iLayer++) {
				...
				m_fn_info.node[iLayer][iNode].coord[0] =
					ratio * (proj_coord[0] - trans_coord[0]) + trans_coord[0];
				m_fn_info.node[iLayer][iNode].coord[1] =
					ratio * (proj_coord[1] - trans_coord[1]) + trans_coord[1];
				m_fn_info.node[iLayer][iNode].coord[2] =
					ratio * (proj_coord[2] - trans_coord[2]) + trans_coord[2];
				m_fn_info.node[iLayer][iNode].idx =
					iNode + m_fn_info.node[0].size() +
					m_fn_info.node[1].size() * (iLayer - 1);
			}
		}
	}
	...
	void GridFNFactoryZaran::BuildProjectNodeNeighbor() {
		...
		for (int iFace = 0; iFace < m_trans_face.size(); iFace++) {
			...
			for (int iNode = 0; iNode < face_node_idx.size(); iNode++) {
				...
				prior_node = face_node_idx[(iNode + face_node_idx.size() - 1) %
					face_node_idx.size()];
		...
		for (int iNode = 0; iNode < m_fn_info.node[1].size(); iNode++) {
			for (int iLayer = 2; iLayer < m_layer_num; iLayer++) {
				int idx = m_fn_info.node[iLayer][iNode].idx;
				m_fn_info.node[iLayer][iNode].neighbor_node.resize(
					node_neighbor_extend[iNode].size() + 2);
				m_fn_info.node[iLayer][iNode].neighbor_node[0] =
					m_fn_info.node[iLayer - 1][iNode].idx;
				m_fn_info.node[iLayer][iNode].neighbor_node[1] =
					m_fn_info.node[iLayer + 1][iNode].idx;
				int id = 2;
				for (auto& neighbor : node_neighbor_extend[iNode]) {
					m_fn_info.node[iLayer][iNode].neighbor_node[id++] =
						m_fn_info.node[iLayer][neighbor].idx;
				}
			}
			m_fn_info.node[m_layer_num][iNode].neighbor_node.resize(
				node_neighbor_extend[iNode].size() + 2);
			m_fn_info.node[m_layer_num][iNode].neighbor_node[0] =
				m_fn_info.node[m_layer_num - 1][iNode].idx;
			m_fn_info.node[m_layer_num][iNode].neighbor_node[1] = -1;
			int id = 2;
			for (auto& neighbor : node_neighbor_extend[iNode]) {
				m_fn_info.node[m_layer_num][iNode].neighbor_node[id++] =
					m_fn_info.node[m_layer_num][neighbor].idx;
			}
		}
	}
	void GridFNFactoryZaran::ReorderProjectNodeNeighbor() {
		...
		for (int iLayer = 2; iLayer < m_layer_num; iLayer++) {
			for (int iNode = 0; iNode < node_num; iNode++) {
				...
				main_vec.normalize();
				neighbor.erase(
					std::find(neighbor.begin(), neighbor.end(), main_pair.node1));
				neighbor.erase(
				...
				for (int i = 0; i < neighbor.size(); ++i) {
					...
					if (node_angle_map.find(angle) != node_angle_map.end()) {
						if (node_proj_map[neighbor[i]].norm() >
							node_proj_map[node_angle_map[angle]].norm()) {
							node_angle_map[angle] = neighbor[i];
							neighbor.erase(std::find(neighbor.begin(), neighbor.end(),
								node_angle_map[angle]));
						}
						else {
							neighbor.erase(
								std::find(neighbor.begin(), neighbor.end(), neighbor[i]));
						}
						--i;
						continue;
					}
				...
				while (neighbor.size() > 4) {
					...
					if (node_dis_map[temp_pair.node1] > node_dis_map[temp_pair.node2]) {
						...
						for (auto& i : node_pair_map) {
							if (i.second.node1 ==
								neighbor[get_last_node(remove_index, neighbor)] &&
						...
						for (auto& i : node_pair_map) {
							if (i.second.node1 == neighbor[remove_index] &&
								i.second.node2 ==
								neighbor[get_next_node(remove_index, neighbor)]) {
								node_pair_map.erase(i.first);
								break;
							}
						}
					...
					vec2 = node_proj_map[temp.node2];
					double angle = AngleOfTwoArray3D(vec1.data(), vec2.data()) +
						GetRand(0.0, 1.0) * EPSILON_NUMBER;
					if (vec1.cross(vec2).dot(main_vec) < 0)
						angle = 2 * PI - angle;
					node_pair_map[angle] = temp;
				}
	...
	void GridFNFactoryZaran::BuildCell() {
		...
		for (int iFace = 0; iFace < m_trans_face.size(); iFace++) {
			...
			for (int iLevel = 1; iLevel < m_layer_num; iLevel++) {
				...
				for (int iNode = 0; iNode < face_node_idx.size(); iNode++) {
					m_fn_info.cell[iCell][iNode] =
						m_fn_info.node[iLevel][face_node_idx[iNode]].idx;
				}
				for (int iNode = 0; iNode < face_node_idx.size(); iNode++) {
					m_fn_info.cell[iCell][iNode + 4] =
						m_fn_info.node[iLevel + 1][face_node_idx[iNode]].idx;
				}
			}
		}
	}
