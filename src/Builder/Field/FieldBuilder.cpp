...
namespace zaran {
	FieldManager* FieldBuilder::Create()
	{
		...
		if (m_grid_type == GridType::Flexible)
		{
			...
			grid_factory = new GridFNFactorySYSU(node_file_name, ele_file_name, bnd_file_name);
		}
		...
		for (int iField = 0; iField < grid_num; iField++) {
			if (m_solver_type == FieldSolverType::NS_FNFDM) {
				...
				if (bound_map->IsBoundaryExist("connection")) {
					...
					for (int i = 0; i < connect_bound.size(); i++) {
						...
						auto recv_grid_struct =
							dynamic_cast<GridStruct*>(grid[recv_field_idx_global[i]]);
			...
			field_data_comm_info[iField] = new FieldDataCommInfo(
