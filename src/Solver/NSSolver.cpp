...
namespace zaran
{
	...
	void NSSolver::InitField()
	{
		...
		VectorXd primInit = para->GetPrimitiveInflow();
		...
		for (int iNode = 0; iNode < nTotalNodeNum; ++iNode)
		{
			...
			x = nodeTopoVec[iNode].GetCoordinate()[0];
			y = nodeTopoVec[iNode].GetCoordinate()[1];
			z = nodeTopoVec[iNode].GetCoordinate()[2];
			x -= 0.5;
			y -= 0.5;
			z -= 0.5;
			if (x * x + y * y + z * z < 0.1 * 0.1)
			{
	...
	void NSSolver::CreateFieldData()
	{
		...
		auto& grid = *GetGrid();
		int nTotalNodeNum = grid.GetTotalNodeNum();
		vector<double>emptyData(nTotalNodeNum);
	...
	void NSSolver::ComputeCoordTrans()
	{
		...
		Coordinate xRight, xLeft, yRight, yLeft, zRight, zLeft;
		for (size_t iNode = 0; iNode < nInnerNode; ++iNode)
		{
	...
	void NSSolver::ComputeGradientWLS()
	{
		...
		for (int iNode = 0; iNode < nInnerNode; ++iNode)
		{
			...
			MatrixXd deltaCoord(3, neighborNodeVec.size());
	...
	void NSSolver::RungeKutta()
	{
		GridPtr grid = GetGrid();
		FlowSolverParaPtr para = GetPara();
		const std::vector<double>& rkCoef = para->GetRKCoef();
		...
		{
			ComputeResidual();
			for (int iNode = 0; iNode < nInnerNode + nBoundNode; ++iNode)
			{
	...
	void NSSolver::InviscidFlux()
	{
		...
		Vector3d r;
		Vector3d grad;
		std::shared_ptr<RiemannSolverPara >fluxPara = std::make_shared<RiemannSolverPara>();
	...
	void NSSolver::ComputeWallBC(Boundary& bound)
	{
		...
		w[boundIndex] = w[innerIndex];
		p[boundIndex] = p[innerIndex];
		Eigen::Vector3d innerVel(u[innerIndex], v[innerIndex], w[innerIndex]);
		Eigen::Vector3d ghostVel = innerVel - 2.0 * (innerVel.dot(boundNorm)) * boundNorm;
